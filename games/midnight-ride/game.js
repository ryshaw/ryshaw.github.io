class MidnightRide extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("landTileset", "./assets/tiled/tilesets/GB-LandTileset.png");
    this.load.image("houseTileset", "./assets/tiled/tilesets/Universal-Buildings-and-walls.png");
    this.load.image("roadTileset", "./assets/tiled/tilesets/Universal-Road-Tileset.png");
  ...
  create() {
    ...
    const groundLayer = map.createLayer("Ground", map.addTilesetImage("land", "landTileset")).setScale(4);
    groundLayer.setPosition(this.width / 2 - groundLayer.width * 2, this.height / 2 - groundLayer.height * 2);

    const roadLayer = map.createLayer("Road", map.addTilesetImage("roads", "roadTileset")).setScale(4);
    roadLayer.setPosition(this.width / 2 - roadLayer.width * 2, this.height / 2 - roadLayer.height * 2);

    const houseLayer = map.createLayer("Houses", map.addTilesetImage("buildings", "houseTileset")).setScale(4);
    houseLayer.setPosition(this.width / 2 - houseLayer.width * 2, this.height / 2 - houseLayer.height * 2);

    const decorLayer = map.createLayer("Decorations", map.addTilesetImage("land", "landTileset")).setScale(4);
    decorLayer.setPosition(this.width / 2 - decorLayer.width * 2, this.height / 2 - decorLayer.height * 2);
    ...
    groundLayer.forEachTile((tile) => {
      roadLayer.hasTileAt(tile.x, tile.y) ? tile.setCollision(false) : tile.setCollision(true);
    });

    this.player = this.physics.add.sprite(this.width / 2, this.height / 2 - 32, "player");
