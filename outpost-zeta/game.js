const VERSION = "Outpost Zeta v0.1";
...
class Game extends Phaser.Scene {
  ...
  create(data) {
    ...
    this.shipColors = [
      0xcdb4db, 0xffc8dd, 0xffafcc, 0xbde0fe, 0xa2d2ff, 0x8affc1,
    ];
  }
  ...
  createPhysics() {
    ...
    this.physics.world.on("worldbounds", (body) => {
      switch (body.gameObject.name) {
        case "ship":
          this.traffic.remove(body.gameObject, true, true);
          break;
      }
    });
  }
  ...
  createTraffic() {
    const delay = Phaser.Math.Between(200, 1800);

    this.time.addEvent({
      delay: delay,
      callback: this.createShip,
      callbackScope: this,
    });

    // loop back so we can keep traffic going
    this.time.addEvent({
      delay: delay,
      callback: this.createTraffic,
      callbackScope: this,
    });
  }
  ...
  loadGameText() {
    new GameText(this, gameW * 0.5, 5, "outpost zeta", "s").setOrigin(0.5, 0);
